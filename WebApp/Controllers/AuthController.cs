...
public class AuthController(UserManager<UserEntity> userManager, SignInManager<UserEntity> signInManager, ApplicationContext context) : Controller
{
    ...
    public async Task<IActionResult> SignUp(SignUpViewModel model)
    {
        if (ModelState.IsValid)
        {
            if (!await _context.Users.AnyAsync(x => x.Email == model.Email))
            {
                ...
                {
                    ViewData["StatusMessage"] = "Somthing is wrong, try later or contact support";
                }
            }
            else
            {
                ViewData["StatusMessage"] = "User with the same email alredy exists";
            }
    ...
    public async Task<IActionResult> SignIn(SignInViewModel model, string returnUrl)
    {
        ...
        ViewData["ReturnUrl"] = returnUrl;
        ViewData["StatusMessage"] = "Incorrect email or passwordd";
        return View(model);
    }
